...
{
    [TestClass]
    public class BitAlgorithmTests
    {
        private BitAlgorithm _algo = null!;

        [TestInitialize]
        public void SetUp()
        {
            _algo = new BitAlgorithm();
        }
        ...
        // ТЕСТ 5: Большие объёмы данных + проверка константного числа операций
        [TestMethod]
        public void Check_LargeData_MaxPower_ConstantOperations()
        {
            long largeNumber = 1073741824; // 2^30
            bool result = _algo.Check(largeNumber);

            Assert.IsTrue(result);
            Assert.AreEqual(4, _algo.OperationCount);
        }
    }
}